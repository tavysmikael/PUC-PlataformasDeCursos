...
const opcoesNota = [
  { value: "1", label: "⭐ 1 — Péssimo" }, { value: "2", label: "⭐⭐ 2 — Ruim" },
...
export default function Avaliacoes() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const jaAvaliou = avaliacoes.some((a) => a.idUsuario === form.idUsuario && a.idCurso === form.idCurso);
    if (jaAvaliou) return setErro("Este usuário já avaliou este curso.");
    try {
      await criarAvaliacao({ idUsuario: form.idUsuario, idCurso: form.idCurso, nota: Number(form.nota), comentario: form.comentario || null, dataAvaliacao: new Date().toLocaleDateString("pt-BR") });
      setForm(estadoInicial); setSucesso("Avaliação registrada com sucesso!"); await carregarDados();
    } catch (error) { setErro(error instanceof Error ? error.message : "Erro ao registrar avaliação."); }
  };

  const handleRemover = async (id: string) => {
    try { await removerAvaliacao(id); await carregarDados(); }
    catch (error) { setErro(error instanceof Error ? error.message : "Erro ao remover avaliação."); }
  };
  ...
  const colunas: Coluna<IAvaliacao>[] = [
    ...
    { key: "nota", label: "Nota", render: (item) => "⭐".repeat(item.nota) },
  ...
  const mediasPorCurso = cursos.map((c) => {
    ...
    const media = avaliacoesCurso.length > 0 ? avaliacoesCurso.reduce((acc, a) => acc + a.nota, 0) / avaliacoesCurso.length : 0;
      ...
      {!carregando && !erroApi && (
        <>
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit}>
                <div className="row">
                  ...
                  <div className="col-md-4"><FormField label="Curso" name="idCurso" type="select" value={form.idCurso} onChange={handleChange} options={opcoesCurso} required /></div>
                  <div className="col-md-4"><FormField label="Nota" name="nota" type="select" value={form.nota} onChange={handleChange} options={opcoesNota} required /></div>
                </div>
                ...
                <ul className="list-group list-group-flush">
                  {mediasPorCurso.map((c, i) => (
                    <li key={c.id} className="list-group-item d-flex justify-content-between align-items-center">
                      <span><span className="fw-bold text-warning me-2">#{i + 1}</span>{c.titulo}</span>
                      <span><span className="badge bg-warning text-dark me-2">{"⭐".repeat(Math.round(c.media))} {c.media.toFixed(1)}</span><span className="badge bg-secondary">{c.totalAvaliacoes} avaliação(ões)</span></span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="card shadow-sm">
            <div className="card-header bg-dark text-white fw-semibold">Avaliações Registradas ({avaliacoes.length})</div>
            <div className="card-body"><DataTable colunas={colunas} dados={avaliacoes} onRemover={handleRemover} /></div>
          </div>
        </>
      )}
    </div>
