...
export default function Trilhas() {
  ...
  const carregarDados = async () => {
    const [t, tc, cat, c] = await Promise.all([
      listarTrilhas(), listarTrilhasCursos(), listarCategorias(), listarCursos(),
    ]);
    setTrilhas(t);
    setTrilhasCursos(tc);
    setCategorias(cat);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSucesso("Trilha cadastrada com sucesso!");
    carregarDados();
  };
  ...
  const handleAdicionarCurso = async () => {
    ...
    const jaExiste = trilhasCursos.some(
      (tc) => tc.idTrilha === trilhaSelecionada.id && tc.idCurso === cursoParaAdicionar
    );
    if (jaExiste) return;

    await criarTrilhaCurso({
      idTrilha: trilhaSelecionada.id,
      idCurso: cursoParaAdicionar,
  ...
  const cursosDaTrilha = trilhaSelecionada
    ? trilhasCursos
        .filter((tc) => tc.idTrilha === trilhaSelecionada.id)
        .sort((a, b) => a.ordem - b.ordem)
        .map((tc) => ({
          ...tc,
          tituloCurso: cursos.find((c) => c.id === tc.idCurso)?.titulo ?? "—",
        }))
    : [];
  ...
  const colunasTrilhas: Coluna<ITrilha>[] = [
    ...
    {
      key: "acoes", label: "Cursos",
      render: (item) => (
        <button className="btn btn-sm btn-outline-warning" onClick={() => setTrilhaSelecionada(item)}>
          🛤️ Gerenciar
        </button>
      ),
    },
  ...
  return (
    <div className="container py-4">
          ...
          <form onSubmit={handleSubmit}>
            <div className="row">
              <div className="col-md-5">
                <FormField label="Título da Trilha" name="titulo" value={form.titulo} onChange={handleChange} placeholder="Ex: Trilha Full Stack" required />
              </div>
              <div className="col-md-4">
                <FormField label="Descrição" name="descricao" value={form.descricao} onChange={handleChange} placeholder="Descreva a trilha..." />
              </div>
        ...
        <div className="card shadow-sm mb-4 border-warning">
          ...
          <div className="card-body">
            <div className="row mb-3">
              ...
              <div className="col-md-3">
                <input className="form-control" type="number" placeholder="Ordem" value={ordemCurso} onChange={(e) => setOrdemCurso(e.target.value)} />
              </div>
              <div className="col-md-2">
                <button className="btn btn-warning w-100 fw-bold" onClick={handleAdicionarCurso}>➕ Adicionar</button>
              </div>
            </div>
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <DataTable colunas={colunasTrilhas} dados={trilhas} onRemover={handleRemover} />
        </div>
      </div>
    </div>
  );
