...
const colunas: Coluna<IPlano>[] = [
  ...
  { key: "descricao", label: "Descrição" },
  {
    ...
    render: (item) => `R$ ${Number(item.preco).toFixed(2)}`,
  },
  ...
  return (
    <div className="container py-4">
          ...
          <form onSubmit={handleSubmit}>
            <div className="row">
              <div className="col-md-4">
                <FormField label="Nome do Plano" name="nome" value={form.nome} onChange={handleChange} placeholder="Ex: Plano Pro" required />
              </div>
              <div className="col-md-4">
                <FormField label="Preço (R$)" name="preco" type="number" value={form.preco} onChange={handleChange} placeholder="Ex: 59.90" required />
              </div>
              <div className="col-md-4">
                <FormField label="Duração (meses)" name="duracaoMeses" type="number" value={form.duracaoMeses} onChange={handleChange} placeholder="Ex: 12" required />
              </div>
            </div>
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <DataTable colunas={colunas} dados={planos} onRemover={handleRemover} />
        </div>
      </div>
    </div>
  );
