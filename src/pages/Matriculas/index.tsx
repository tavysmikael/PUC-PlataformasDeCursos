...
export default function Matriculas() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const jaMatriculado = matriculas.some((m) => m.idUsuario === form.idUsuario && m.idCurso === form.idCurso);
    if (jaMatriculado) return setErro("Este usuário já está matriculado neste curso.");
    try {
      await criarMatricula({ idUsuario: form.idUsuario, idCurso: form.idCurso, dataMatricula: new Date().toLocaleDateString("pt-BR"), dataConclusao: null });
      setForm(estadoInicial); setSucesso("Matrícula realizada com sucesso!"); await carregarDados();
    } catch (error) { setErro(error instanceof Error ? error.message : "Erro ao realizar matrícula."); }
  };

  const handleRemover = async (id: string) => {
    try { await removerMatricula(id); await carregarDados(); }
    catch (error) { setErro(error instanceof Error ? error.message : "Erro ao remover matrícula."); }
  };
  ...
  const colunas: Coluna<IMatricula>[] = [
    ...
    { key: "dataConclusao", label: "Conclusão", render: (item) => item.dataConclusao ?? "⏳ Em andamento" },
  ];
      ...
      {!carregando && !erroApi && (
        <>
          <div className="card shadow-sm mb-4">
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit}>
                <div className="row">
                  <div className="col-md-6"><FormField label="Aluno" name="idUsuario" type="select" value={form.idUsuario} onChange={handleChange} options={opcoesUsuario} required /></div>
                  <div className="col-md-6"><FormField label="Curso" name="idCurso" type="select" value={form.idCurso} onChange={handleChange} options={opcoesCurso} required /></div>
                </div>
          ...
          <div className="card shadow-sm">
            <div className="card-header bg-dark text-white fw-semibold">Matrículas Realizadas ({matriculas.length})</div>
            <div className="card-body"><DataTable colunas={colunas} dados={matriculas} onRemover={handleRemover} /></div>
          </div>
        </>
      )}
    </div>
