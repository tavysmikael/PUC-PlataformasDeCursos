...
const estadoInicial = { titulo: "", descricao: "", idInstrutor: "", idCategoria: "", nivel: "", totalAulas: "", totalHoras: "" };
...
export default function Cursos() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      setCursos(c); setUsuarios(u); setCategorias(cat);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await criarCurso({
        titulo: form.titulo, descricao: form.descricao, idInstrutor: form.idInstrutor,
        idCategoria: form.idCategoria, nivel: form.nivel, totalAulas: Number(form.totalAulas),
        totalHoras: Number(form.totalHoras), dataPublicacao: new Date().toLocaleDateString("pt-BR"),
      });
      setForm(estadoInicial);
      setSucesso("Curso cadastrado com sucesso!");
      await carregarDados();
    } catch (error) {
      setErro(error instanceof Error ? error.message : "Erro ao cadastrar curso.");
    }
  };

  const handleRemover = async (id: string) => {
    try { await removerCurso(id); await carregarDados(); } catch (error) {
      setErro(error instanceof Error ? error.message : "Erro ao remover curso.");
    }
  };
  ...
  const colunas: Coluna<ICurso>[] = [
    ...
    { key: "idInstrutor", label: "Instrutor", render: (item) => usuarios.find((u) => u.id === item.idInstrutor)?.nomeCompleto ?? "—" },
      ...
      {!carregando && !erroApi && (
        <>
          <div className="card shadow-sm mb-4">
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit}>
                <div className="row">
                  <div className="col-md-6"><FormField label="Título do Curso" name="titulo" value={form.titulo} onChange={handleChange} placeholder="Ex: React do Zero ao Avançado" required /></div>
                  <div className="col-md-3"><FormField label="Instrutor" name="idInstrutor" type="select" value={form.idInstrutor} onChange={handleChange} options={opcoesInstrutor} required /></div>
                ...
                <div className="row">
                  <div className="col-md-4"><FormField label="Nível" name="nivel" type="select" value={form.nivel} onChange={handleChange} options={niveis} required /></div>
                  <div className="col-md-4"><FormField label="Total de Aulas" name="totalAulas" type="number" value={form.totalAulas} onChange={handleChange} placeholder="Ex: 40" /></div>
                  <div className="col-md-4"><FormField label="Total de Horas" name="totalHoras" type="number" value={form.totalHoras} onChange={handleChange} placeholder="Ex: 20" /></div>
                </div>
          ...
          <div className="card shadow-sm">
            ...
            <div className="card-body"><DataTable colunas={colunas} dados={cursosFiltrados} onRemover={handleRemover} /></div>
          </div>
        </>
      )}
    </div>
