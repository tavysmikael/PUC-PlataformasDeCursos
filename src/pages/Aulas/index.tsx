...
const estadoInicial = {
  idModulo: "", titulo: "", tipoConteudo: "",
  urlConteudo: "", duracaoMinutos: "", ordem: "",
};
...
export default function Aulas() {
  ...
  const opcoesModulo = modulosDoCurso.map((m) => ({
    value: m.id,
    label: `Módulo ${m.ordem} — ${m.titulo}`,
  }));
  ...
  const colunas: Coluna<IAula>[] = [
    ...
    {
      key: "idModulo", label: "Módulo",
      render: (item) => modulos.find((m) => m.id === item.idModulo)?.titulo ?? "—",
    },
    {
      key: "urlConteudo", label: "Link",
      render: (item) =>
        item.urlConteudo ? (
          <a href={item.urlConteudo} target="_blank" rel="noreferrer">🔗 Acessar</a>
        ) : "—",
    },
  ];
  ...
  return (
    <div className="container py-4">
        ...
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit}>
            <div className="row">
              <div className="col-md-6">
                <div className="mb-3">
                  <label className="form-label fw-semibold">
                    Filtrar por Curso <span className="text-muted fw-normal">(para carregar módulos)</span>
                  </label>
              ...
              <div className="col-md-6">
                <FormField label="Módulo" name="idModulo" type="select" value={form.idModulo} onChange={handleChange} options={opcoesModulo} required />
              </div>
            </div>
            <div className="row">
              <div className="col-md-5">
                <FormField label="Título da Aula" name="titulo" value={form.titulo} onChange={handleChange} placeholder="Ex: O que é JSX?" required />
              </div>
              <div className="col-md-3">
                <FormField label="Tipo de Conteúdo" name="tipoConteudo" type="select" value={form.tipoConteudo} onChange={handleChange} options={tiposConteudo} required />
              </div>
              <div className="col-md-2">
                <FormField label="Duração (min)" name="duracaoMinutos" type="number" value={form.duracaoMinutos} onChange={handleChange} placeholder="Ex: 15" />
              </div>
              <div className="col-md-2">
                <FormField label="Ordem" name="ordem" type="number" value={form.ordem} onChange={handleChange} placeholder="Ex: 1" required />
              </div>
            </div>
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <DataTable colunas={colunas} dados={aulasOrdenadas} onRemover={handleRemover} />
        </div>
      </div>
    </div>
  );
